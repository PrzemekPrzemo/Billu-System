...
<div class="card" style="margin-bottom:20px;padding:16px;">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
        ...
        <div style="padding:8px 12px;background:#f5f5f5;border-radius:6px;display:flex;align-items:center;gap:8px;">
            ...
            <form method="POST" action="/office/import/template-delete/<?= $tpl['id'] ?>" style="display:inline;">
...
<details style="margin-bottom:16px;">
    ...
    <div class="card" style="padding:16px;margin-top:8px;">
        <form method="POST" action="/office/import/template-save">
            <input type="hidden" name="csrf_token" value="<?= $csrf ?>">
            <div class="form-row">
...
<div class="section">
    <h2><?= $lang('file_import') ?></h2>
    <form method="POST" action="/office/import" enctype="multipart/form-data" class="form-card">
        <input type="hidden" name="csrf_token" value="<?= $csrf ?>">
...
<?php if (\App\Models\Setting::get('ksef_api_env')): ?>
<div class="section">
    <h2><?= $lang('ksef_import') ?></h2>
    <form method="POST" action="/office/import/ksef" class="form-card" id="ksef-import-form">
        <input type="hidden" name="csrf_token" value="<?= $csrf ?>">
    ...
    (function() {
        ...
        var statusUrl = '/office/import/ksef-status';
        ...
        form.addEventListener('submit', function() {
            btn.disabled = true; progress.style.display = 'block'; resultDiv.style.display = 'none';
        });

        function startPolling(id) {
            btn.disabled = true; progress.style.display = 'block'; resultDiv.style.display = 'none'; poll(id);
        }
        function poll(id) {
            fetch(statusUrl + '?job_id=' + encodeURIComponent(id))
                ...
                .then(function(data) {
                    if (data.status === 'running' || data.status === 'queued') {
                        statusText.textContent = data.message || 'Importowanie...';
                        setTimeout(function() { poll(id); }, 2000);
                    } else { progress.style.display = 'none'; btn.disabled = false; showResult(data); }
                })
                .catch(function() { progress.style.display = 'none'; btn.disabled = false; resultDiv.style.display = 'block'; resultDiv.innerHTML = '<div class="alert alert-error">Błąd połączenia</div>'; });
        }
        function showResult(data) {
            resultDiv.style.display = 'block';
            var r = data.result || {};
            if (data.status === 'error') { resultDiv.innerHTML = '<div class="alert alert-error">' + esc(data.message) + '</div>'; }
            else if (r.success > 0) { resultDiv.innerHTML = '<div class="alert alert-success">' + esc(data.message) + '</div>'; }
            else { resultDiv.innerHTML = '<div class="alert alert-warning">' + esc(data.message) + '</div>'; }
        }
