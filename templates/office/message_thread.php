<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; flex-wrap:wrap; gap:8px;">
    ...
    <a href="/office/messages" class="btn btn-sm">&larr; <?= $lang('back_to_messages') ?></a>
</div>

<div style="margin-bottom:12px; color:var(--text-muted); font-size:13px;">
    <?= $lang('client') ?>: <strong><?= htmlspecialchars($client['company_name'] ?? '') ?></strong>
    <?php if ($thread['invoice_id']): ?>
        &middot; <?= $lang('invoice') ?> #<?= (int) $thread['invoice_id'] ?>
    <?php endif; ?>
</div>
    ...
    <?php foreach ($messages as $m): ?>
        <div class="message-bubble <?= $m['sender_type'] === 'client' ? 'msg-from-client' : 'msg-from-office' ?>">
            ...
            <div style="margin-top:8px; padding-top:6px; border-top:1px solid var(--gray-200);">
                <a href="/office/messages/attachment/<?= $m['id'] ?>" style="display:inline-flex; align-items:center; gap:4px; font-size:13px; color:var(--primary);">
...
<div class="card" style="margin-top:16px;">
    <div class="card-body">
        <form method="post" action="/office/messages/<?= $thread['id'] ?>/reply" enctype="multipart/form-data">
            <input type="hidden" name="csrf_token" value="<?= $csrf ?>">
            <div class="form-group" style="margin-bottom:12px;">
                <label class="form-label"><?= $lang('reply') ?></label>
                <textarea name="body" class="form-input" rows="3" required placeholder="<?= $lang('write_reply') ?>..."></textarea>
            </div>
            <div class="form-group" style="margin-bottom:12px;">
