...
<div class="section-header">
    <h1>Lista plac <?= sprintf('%02d/%04d', $list['month'], $list['year']) ?> - <?= htmlspecialchars($client['company_name']) ?></h1>
    <div style="display:flex; gap:8px;">
        <a href="/office/hr/<?= $client['id'] ?>/payroll" class="btn btn-secondary"><?= $lang('back') ?></a>
        <a href="/office/hr/<?= $client['id'] ?>/payroll/<?= $list['id'] ?>/pdf" class="btn btn-primary">
        ...
        <form method="POST" action="/office/hr/<?= $client['id'] ?>/payroll/<?= $list['id'] ?>/approve" class="inline-form">
            ...
            <button type="submit" class="btn btn-primary" onclick="return confirm('Zatwierdzic liste plac?')">Zatwierdz</button>
        </form>
        <?php endif; ?>
    </div>
</div>
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href="/office/hr/<?= $client['id'] ?>/payroll/<?= $list['id'] ?>/payslip/<?= $e['id'] ?>/pdf" class="btn btn-xs" title="Pobierz pasek placowy">
