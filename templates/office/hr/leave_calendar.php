<div class="section-header">
    <div>
        <div class="breadcrumb-path" style="font-size:13px;color:var(--text-muted);margin-bottom:4px;">
            <a href="/office/hr/<?= $clientId ?>/leaves"><?= $lang('hr_leave_requests') ?></a> &rsaquo;
            <?= $lang('hr_leave_calendar') ?>
        </div>
        <h1><?= $lang('hr_leave_calendar') ?> — <?= htmlspecialchars($client['company_name']) ?></h1>
    </div>
    <a href="/office/hr/<?= $clientId ?>/leaves" class="btn btn-secondary"><?= $lang('back') ?></a>
</div>
...
<div class="card">
    <div class="card-header" style="display:flex;align-items:center;gap:16px;">
        <a href="/office/hr/<?= $clientId ?>/leaves/calendar?month=<?= $prevMonth ?>&year=<?= $prevYear ?>" class="btn btn-sm btn-secondary">&lsaquo;</a>
        <h3 style="margin:0;flex:1;text-align:center;"><?= $months[$month] ?> <?= $year ?></h3>
        <a href="/office/hr/<?= $clientId ?>/leaves/calendar?month=<?= $nextMonth ?>&year=<?= $nextYear ?>" class="btn btn-sm btn-secondary">&rsaquo;</a>
    </div>

    <?php if (!empty($employees)): ?>
    <div class="card-body" style="padding:8px 16px 4px;">
        <label for="filter-emp" style="font-size:13px;color:var(--text-muted);">Pracownik:</label>
