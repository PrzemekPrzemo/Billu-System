...
<div class="form-card" style="padding:20px; margin-bottom:20px;">
    ...
    <form method="POST" action="/client/files/upload" enctype="multipart/form-data">
...
<div class="form-card" style="padding:12px 16px; margin-bottom:20px;">
    <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:center;">
        ...
        <?php foreach ($categories as $key => $cat): ?>
            <a href="/client/files?category=<?= $key ?>"
        ...
        <table class="table table-striped">
            ...
            <tbody>
                <?php foreach ($files as $f): ?>
                    ...
                    <tr>
                        <td>
                            <a href="/client/files/<?= (int)$f['id'] ?>/download" style="text-decoration:none;">
                        ...
                        <td style="white-space:nowrap;">
                            <a href="/client/files/<?= (int)$f['id'] ?>/download" class="btn btn-sm btn-primary" title="Pobierz">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:middle;">
                                    <path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/>
                                </svg>
                            </a>
                            <?php
                                $clientId = \App\Core\Session::get('client_id');
                                $canDelete = ($f['uploaded_by_type'] === 'client' && (int)$f['uploaded_by_id'] === (int)$clientId);
                            ?>
                            <?php if ($canDelete): ?>
                                <form method="POST" action="/client/files/<?= (int)$f['id'] ?>/delete" style="display:inline;"
