<div class="page-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;">
    <div>
        ...
        <p style="color:var(--gray-500);margin-top:4px;">
            <?= $lang('modules_for_office') ?>: <strong><?= htmlspecialchars($office['name']) ?></strong>
            (NIP: <?= htmlspecialchars($office['nip']) ?>)
        </p>
    </div>
    <a href="/admin/offices/<?= $office['id'] ?>/edit" class="btn btn-secondary">&larr; <?= $lang('back_to_edit') ?></a>
</div>

<form method="POST" action="/admin/offices/<?= $office['id'] ?>/modules" class="form-card">
    ...
    <?php foreach ($categories as $catSlug => $catName): ?>
        ...
        <div style="margin-bottom:24px;">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:12px;">
                ...
                ?>
                <label class="module-card" style="display:flex;align-items:center;gap:12px;padding:14px 16px;border-radius:var(--radius-md,8px);border:1px solid var(--gray-200);cursor:<?= $isSystem ? 'default' : 'pointer' ?>;background:var(--gray-50);transition:border-color 0.15s;<?= $isEnabled ? 'border-color:var(--primary);' : '' ?>">
                    ...
                    <div style="flex-shrink:0;font-size:11px;font-weight:600;<?= $isEnabled ? 'color:var(--success);' : 'color:var(--gray-400);' ?>">
                        <?= $isEnabled ? $lang('module_enabled') : $lang('module_disabled') ?>
                    </div>
                </label>
                <?php endforeach; ?>
            </div>
    ...
    <div class="form-actions" style="margin-top:24px;">
        ...
        <a href="/admin/offices/<?= $office['id'] ?>/edit" class="btn btn-secondary"><?= $lang('cancel') ?></a>
    </div>
</form>
