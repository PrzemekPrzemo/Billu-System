...
4. Inspektor Ochrony Danych (IOD): Administrator nie wyznaczyl Inspektora Ochrony Danych
   na podstawie art. 37 RODO. Kontakt w sprawach dotyczacych ochrony danych osobowych jest
   mozliwy pod adresem e-mail: info@billu.pl.

5. Administrator doklada szczegolnej starannosci w celu ochrony interesow osob, ktorych dane
...
4. **Klient** - przedsiebiorca korzystajacy z Serwisu w celu weryfikacji faktur zakupowych,
   powiazany z Biurem Ksiegowym.
5. **Uzytkownik** - kazda osoba korzystajaca z Serwisu (Administrator, Biuro Ksiegowe, Klient).
6. **Dane osobowe** - informacje o zidentyfikowanej lub mozliwej do zidentyfikowania osobie fizycznej.
7. **RODO** - Rozporzadzenie Parlamentu Europejskiego i Rady (UE) 2016/679 z dnia 27 kwietnia 2016 r.

---

## III. ROLE W PRZETWARZANIU DANYCH

### 3.1 Operator jako Administrator danych

Operator jest Administratorem danych osobowych w zakresie:
- danych rejestracyjnych uzytkownikow Serwisu (login, haslo, e-mail),
- danych kontaktowych przedstawicieli Biur Ksiegowych i Klientow,
- danych technicznych (adresy IP, logi sesji, historia logowan),
- danych rozliczeniowych zwiazanych z subskrypcja.

### 3.2 Operator jako Podmiot przetwarzajacy (Procesor)

Operator dziala jako Podmiot przetwarzajacy w rozumieniu art. 28 RODO w zakresie danych
zawartych w fakturach importowanych do Serwisu, na podstawie:

a) **Umowy powierzenia przetwarzania danych z Biurem Ksiegowym** - w zakresie faktur
   importowanych przez Biuro Ksiegowe w imieniu swoich klientow.

b) **Umowy powierzenia przetwarzania danych z Klientem** - w zakresie faktur importowanych
   bezposrednio przez Klienta (w tym z systemu KSeF).

### 3.3 Biuro Ksiegowe jako Administrator danych

Biuro Ksiegowe jest Administratorem danych osobowych zawartych w fakturach swoich klientow
i powierza ich przetwarzanie Operatorowi na podstawie umowy powierzenia.

### 3.4 Klient jako Administrator danych

Klient jest Administratorem danych osobowych zawartych w jego fakturach zakupowych
i powierza ich przetwarzanie Operatorowi na podstawie umowy powierzenia.
...
|-------------------|----------------|---------------------|
| Rejestracja i obsluga konta | Art. 6 ust. 1 lit. b RODO (wykonanie umowy) | Czas trwania umowy + 6 miesiecy |
| Uwierzytelnianie i bezpieczenstwo | Art. 6 ust. 1 lit. f RODO (uzasadniony interes) | Czas trwania sesji + 30 dni |
| Rozliczenia i fakturowanie | Art. 6 ust. 1 lit. c RODO (obowiazek prawny) | 5 lat od konca roku podatkowego |
| Dziennik audytu i logi | Art. 6 ust. 1 lit. f RODO (uzasadniony interes) | Czas trwania umowy + 6 miesiecy |
| Historia logowan | Art. 6 ust. 1 lit. f RODO (uzasadniony interes) | 12 miesiecy |
| Kontakt i obsluga | Art. 6 ust. 1 lit. b RODO (wykonanie umowy) | Czas trwania umowy + 6 miesiecy |

### 4.2 Dane przetwarzane jako Procesor (na zlecenie)

| Kategoria danych | Zrodlo | Cel | Podstawa |
|-------------------|--------|-----|----------|
| Dane z faktur (NIP, nazwa, adres sprzedawcy/nabywcy) | Import plikow / KSeF | Obieg i weryfikacja faktur | Umowa powierzenia (art. 28 RODO) |
| Kwoty, daty, numery faktur | Import plikow / KSeF | Raportowanie i eksport | Umowa powierzenia (art. 28 RODO) |
| Komentarze weryfikacyjne | Klient | Dokumentacja decyzji | Umowa powierzenia (art. 28 RODO) |
| Certyfikaty i tokeny KSeF | Klient | Integracja z KSeF | Umowa powierzenia (art. 28 RODO) |
...
a) **Krajowy System e-Faktur (KSeF)** - Ministerstwo Finansow RP - w zakresie niezbednym
   do pobierania i weryfikacji e-faktur (na podstawie upoważnienia Klienta).

b) **GUS (Glowny Urzad Statystyczny)** - w zakresie weryfikacji danych rejestrowych firm
   (NIP, REGON, adres) - dane publiczne.

c) **Dostawca hostingu** - podmiot swiadczacy uslugi hostingowe na podstawie umowy powierzenia
   przetwarzania danych. Serwery zlokalizowane w Polsce/UE.
   <!-- KONFIGURACJA: Podaj nazwe dostawcy hostingu -->

d) **Dostawca uslug backup** - podmiot swiadczacy uslugi kopii zapasowych na podstawie
   umowy powierzenia. Kopie zapasowe przechowywane w Polsce/UE.
   <!-- KONFIGURACJA: Podaj nazwe dostawcy backup -->

e) **Dostawca uslug monitoringu** - podmiot swiadczacy uslugi monitorowania infrastruktury
   na podstawie umowy powierzenia (jesli dotyczy).
   <!-- KONFIGURACJA: Podaj nazwe dostawcy monitoringu lub usun punkt jesli nie dotyczy -->

f) **Dostawca uslug SMTP (poczty e-mail)** - podmiot swiadczacy uslugi poczty elektronicznej
   w zakresie adresow e-mail odbiorcow wiadomosci systemowych.
   <!-- KONFIGURACJA: Podaj nazwe dostawcy SMTP -->
...
Dane osobowe przetwarzane w Serwisie **nie sa przekazywane** poza Europejski Obszar
Gospodarczy (EOG). Wszystkie serwery, kopie zapasowe i systemy przetwarzajace dane
znajduja sie na terytorium Polski lub Unii Europejskiej.
...
| Kategoria danych | Okres przechowywania | Forma |
|-------------------|---------------------|-------|
| Dane z faktur (produkcja) | Usuwane natychmiast po zakonczeniu umowy | Usuniete z serwera produkcyjnego |
| Dane z faktur (backup) | 6 miesiecy od zakonczenia umowy | Kopia zapasowa (szyfrowana) |
| Dane rejestracyjne | 6 miesiecy od zakonczenia umowy | Kopia zapasowa |
| Dane rozliczeniowe | 5 lat od konca roku podatkowego | Archiwum (obowiazek podatkowy) |
| Logi i dziennik audytu | 6 miesiecy od zakonczenia umowy | Kopia zapasowa |
...
### 9.8 Realizacja praw
Wnioski dotyczace realizacji praw nalezy kierowac na adres: info@billu.pl
...
| Element | Metoda szyfrowania |
|---------|-------------------|
| Komunikacja sieciowa | TLS 1.2+ (HTTPS) z wymuszeniem HSTS |
| Tokeny KSeF w bazie danych | AES-256-GCM z kluczem per-instalacja |
| Certyfikaty kwalifikowane | AES-256-GCM z kluczem per-instalacja |
| Hasla uzytkownikow | bcrypt z cost factor 12 (jednokierunkowy hash) |
| Kopie zapasowe | Szyfrowanie na poziomie infrastruktury |
...
### 11.1 Serwis wykorzystuje wylacznie cookies niezbedne:

| Nazwa cookie | Cel | Typ | Czas zycia |
|-------------|-----|-----|------------|
| PHPSESSID | Identyfikator sesji uzytkownika | Niezbedne | Do zamkniecia przegladarki lub 30 min nieaktywnosci |

### 11.2 Serwis NIE wykorzystuje:

- Cookies analitycznych (Google Analytics, itp.)
- Cookies reklamowych
- Cookies sledzacych
- Pikselow sledzacych
...
Operator przetwarza dane osobowe zawarte w fakturach na podstawie umowy powierzenia
przetwarzania danych (art. 28 RODO), zawartej:

a) z Biurem Ksiegowym - w zakresie danych klientow Biura Ksiegowego,
b) z Klientem - w zakresie danych z faktur importowanych bezposrednio przez Klienta.

### 12.2 Zakres powierzenia

Operator przetwarza powierzone dane wylacznie w celu:
...
### 12.3 Obowiazki Operatora jako Procesora

Operator zobowiazuje sie do:
...
Operator korzysta z nastepujacych podprocesorow:

| Podprocesor | Zakres | Lokalizacja |
|-------------|--------|-------------|
| Dostawca hostingu <!-- KONFIGURACJA: Podaj nazwe --> | Przechowywanie danych na serwerach | Polska / UE |
| Dostawca backup <!-- KONFIGURACJA: Podaj nazwe --> | Kopie zapasowe | Polska / UE |
| Dostawca SMTP <!-- KONFIGURACJA: Podaj nazwe --> | Przesylanie wiadomosci e-mail | Polska / UE |
| Ministerstwo Finansow RP (KSeF) | Pobieranie e-faktur | Polska |
| GUS (BIR1 API) | Weryfikacja danych rejestrowych | Polska |
...
1. **W ciagu 24 godzin** - powiadomi wlasciwego Administratora danych (Biuro Ksiegowe / Klienta).
...
3. Udokumentuje naruszenie, jego skutki i podjete dzialania naprawcze.

### 13.2 Kontakt w sprawie incydentu

Zgloszenia incydentow bezpieczenstwa: info@billu.pl

---

## XIV. ZMIANY POLITYKI PRYWATNOSCI

1. Operator zastrzega sobie prawo do zmiany niniejszej Polityki Prywatnosci.
...
3. Wszelkie spory beda rozstrzygane przez sad wlasciwy dla siedziby Operatora.
