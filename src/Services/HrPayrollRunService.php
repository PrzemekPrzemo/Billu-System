...
class HrPayrollRunService
{
    public static function calculateRun(int $runId, string $actorType, int $actorId): array
    {
        ...
        if ($run['status'] === 'locked') {
            return ['error' => 'Payroll run is locked and cannot be recalculated'];
        }
        ...
        $totals = [
            ...
            'total_zus_employee' => 0.0, 'total_zus_employer' => 0.0,
            'total_pit_advance' => 0.0, 'total_ppk_employee' => 0.0,
            'total_ppk_employer' => 0.0, 'total_employer_cost' => 0.0,
            'employee_count' => 0,
        ];
        $errors = [];
        ...
        foreach ($employees as $emp) {
            try {
                ...
                $overrides = $existingOverrides[$emp['id']] ?? [
                    'overtime_pay' => 0.0, 'bonus' => 0.0,
                    'other_additions' => 0.0, 'sick_pay_reduction' => 0.0,
                ];

                $employeeArr = [
                    'pit2_submitted'    => $emp['pit2_submitted'],
                    'kup_amount'        => $emp['kup_amount'],
                    'ppk_enrolled'      => $emp['ppk_enrolled'],
                    'ppk_employee_rate' => $emp['ppk_employee_rate'],
                    'ppk_employer_rate' => $emp['ppk_employer_rate'],
                ];
                $contractArr = [
                    'contract_type'           => $emp['contract_type'],
                    'base_salary'             => $emp['base_salary'],
                    'work_time_fraction'      => $emp['work_time_fraction'],
                    'zus_emerytalne_employee' => $emp['zus_emerytalne_employee'],
                    'zus_emerytalne_employer' => $emp['zus_emerytalne_employer'],
                    'zus_rentowe_employee'    => $emp['zus_rentowe_employee'],
                    'zus_rentowe_employer'    => $emp['zus_rentowe_employer'],
                    'zus_chorobowe'           => $emp['zus_chorobowe'],
                    'zus_wypadkowe'           => $emp['zus_wypadkowe'],
                    'zus_fp'                  => $emp['zus_fp'],
                    'zus_fgsp'                => $emp['zus_fgsp'],
                    'wypadkowe_rate'          => $emp['wypadkowe_rate'],
                    'has_other_employment'    => $emp['has_other_employment'],
                ];

                $result = HrPayrollCalculationService::calculate($employeeArr, $contractArr, $settings, $overrides, $ytd);

                HrPayrollItem::upsert(array_merge($result, [
        ...
        AuditLog::log($actorType, $actorId, 'hr_payroll_calculate',
            json_encode(['run_id' => $runId, 'employee_count' => $totals['employee_count']]),
    ...
    public static function unlockRun(int $runId, string $reason, string $actorType, int $actorId): void
    {
        ...
        if ($run['status'] !== 'locked') {
            throw new \RuntimeException("Tylko zablokowane listy p\u0142ac mog\u0105 by\u0107 odblokowane. Status: {$run['status']}");
        }

        HrPayrollRun::unlock($runId, $reason, $actorType, $actorId);

        AuditLog::log($actorType, $actorId, 'hr_payroll_unlock',
            json_encode(['run_id' => $runId, 'reason' => $reason]),
            'hr_payroll_run', $runId
        );
    }

    public static function createCorrectionRun(
        int $originalRunId, array $employeeIds, string $actorType, int $actorId
    ): int {
        $originalRun = HrPayrollRun::findById($originalRunId);
        if (!$originalRun) {
            throw new \RuntimeException("Nie znaleziono oryginalnej listy p\u0142ac: {$originalRunId}");
        }
        ...
        foreach ($employeeIds as $empId) {
            ...
            $originalItem = $db->fetchOne(
                "SELECT * FROM hr_payroll_items WHERE payroll_run_id = ? AND employee_id = ?",
                [$originalRunId, $empId]
            );
        ...
        AuditLog::log($actorType, $actorId, 'hr_payroll_correction_create',
            json_encode(['original_run_id' => $originalRunId, 'correction_run_id' => $correctionRunId, 'employee_ids' => $employeeIds]),
            'hr_payroll_run', $correctionRunId
        );
    ...
    public static function applyYtdImpact(int $correctionRunId): void
    {
        ...
        $corrItems = $db->fetchAll(
            "SELECT pi.*, orig.gross_salary AS orig_gross,
                    orig.pit_advance AS orig_pit, orig.zus_employee AS orig_zus
        ...
        $laterRuns = $db->fetchAll(
            "SELECT id, period_month FROM hr_payroll_runs
             WHERE client_id = ? AND period_year = ? AND period_month > ?
               AND is_correction = 0 AND status != 'draft'
        ...
        foreach ($corrItems as $item) {
            ...
            foreach ($laterRuns as $laterRun) {
                $laterItem = $db->fetchOne(
                    "SELECT id, ytd_gross, ytd_zus_base FROM hr_payroll_items
                     WHERE payroll_run_id = ? AND employee_id = ?",
                    [(int)$laterRun['id'], $empId]
                );
